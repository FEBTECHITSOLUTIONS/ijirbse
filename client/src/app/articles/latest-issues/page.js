...
export default function LatestIssues() {
  ...
  useEffect(() => {
    const fetchLatestIssue = async () => {
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/api/latest-issue`);
        console.log(res);
        ...
        {issues.map((issue, i) => {
          ...
          return (
            ...
            <span className="text-blue-700 flex items-center gap-1">
               <MdLink /> <a className=" " target="_blank" href={`${process.env.NEXT_PUBLIC_BASE_URL}${article.uploadedManuscriptMetaData?.url}`}>Read Article</a>
            </span>
